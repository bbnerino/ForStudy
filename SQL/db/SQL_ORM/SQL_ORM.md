...
2. user 레코드 생성
   ...
   ```python
   # orm
   User.objects.create(first_name='길동',last_name='홍', age=100,country='제주도',phone='010-3144-5102', balance=10000)
   ```

   ```sql
   -- sql
   INSERT INTO users_user VALUES(102,'길동','김',21,'부산','010-5152-9855',8889);
   ```
...
4. 해당 user 레코드 수정
   ...
   ```
      ...
      ```sql
   -- sql
   UPDATE users_user SET first_name='현홍' WHERE id=102;
      ```
...
3. 나이가 30살 이상인 사람의 인원 수
   ...
   -  greater than equal 이상 , 이하 / 초과 , 미만
...
1. 나이가 많은 사람순으로 10명
   ...
   ```python
   # orm
   User.objects.order_by('-age')[:10].values('first_name','age')
   ```
...
3. 잔고는 오름차순, 나이는 내림차순으로 10명?
      ...
      ```python
   # orm
   User.objects.order_by('balance','-age')[:10].values('first_name','balance','age')
   ```
...
6. 지역 별 인원수 
   ...
   User.objects.values('country').annotate(num_countries=Count('country'),avg_balance=Avg('balance'))
   ```
